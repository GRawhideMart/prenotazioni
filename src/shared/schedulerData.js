...
export const SCHEDULERDATA = [
  {
    ...
    title: PROGRAMS.filter((program) => program.tag === "domanialluna")[0]
      .title,
    backgroundColor: PROGRAMS.filter(
      (program) => program.tag === "domanialluna"
    )[0].background_color,
    color: PROGRAMS.filter((program) => program.tag === "domanialluna")[0]
      .color,
    backgroundImage: PROGRAMS.filter(
      (program) => program.tag === "domanialluna"
    )[0].background_image,
